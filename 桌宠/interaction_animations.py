...
class InteractionAnimationMixin:
    ...
    def _paste_climb_hands(self, canvas):
        ...
        hw, hh = hand.size
        t = max(0.0, min(1.0, self._climb_hand_progress))
        edge_y = max(0, min(canvas.height - hh, self._climb_edge_y - hh // 2))
        target_lx = max(0, lx - int(22 * t))
        target_rx = min(canvas.width - hw, rx + int(22 * t))
    ...
    def _paste_scratch_hands(self, canvas):
        ...
        lift = getattr(self, '_scratch_lift', 1.0)
        lift_y = int(hy + (target_y - hy) * max(0.0, min(1.0, lift)))
        if self.facing_right:
        ...
        if self._scratch_side == 'left':
            ...
            canvas.paste(left_hand, (lift_x, lift_y), left_hand)
        else:
            canvas.paste(left_hand, (lx, hy), left_hand)
            canvas.paste(right_hand, (lift_x, lift_y), right_hand)
    ...
    def _jump_air_tick(self):
        ...
        self._jump_job = self.animations.schedule('jump', JUMP_FRAME_MS, self._jump_air_tick)
    ...
    def _wall_slide_tick(self):
        ...
        if y >= ground:
            ...
            return
        self.root.geometry(f'{size}x{size}+{x}+{y}')
        self._jump_job = self.animations.schedule('jump', JUMP_FRAME_MS, self._wall_slide_tick)
    ...
    def _hop_tick(self):
        ...
        self.root.geometry(f'{size}x{size}+{x}+{y}')
        self._jump_job = self.animations.schedule('jump', JUMP_FRAME_MS, self._hop_tick)
    ...
    def _record_drag_sample(self, event):
        ...
        cutoff = now - 0.16
        self._drag_samples = [
            sample for sample in self._drag_samples if sample[0] >= cutoff]
    ...
    def _begin_sway(self, event):
        ...
        self._anchor_sway(event)
        self._sway_job = self.animations.schedule('sway', SWAY_FRAME_MS, self._sway_tick)
    ...
    def _start_airborne_motion(self, vx, vy):
        ...
        if not self.motion.enter('fling', force=True):
            return False
        size = int(ICON_SIZE * self.scale)
        self._airborne_x = self.root.winfo_x()
        self._airborne_y = self.root.winfo_y()
        self._fling_vx = max(-AIRBORNE_MAX_SPEED,
                             min(AIRBORNE_MAX_SPEED, vx))
        ...
        self._cancel_anim()
        self._on_taskbar = False
        self.facing_right = self._fling_vx >= 0
        self.pose = 'drag'
        initial_key = self._airborne_visual_key()
        self._airborne_last_visual_key = initial_key
        self._airborne_next_render_at = time.monotonic() + AIRBORNE_RENDER_MS / 1000.0
    ...
    def _fling_tick(self):
        ...
        edge_overshoot = int(size * FLING_EDGE_OVERSHOOT_RATIO)
        left_limit = -int(size * CAT_LEFT_RATIO) - edge_overshoot
        right_limit = (screen_w - int(size * CAT_RIGHT_RATIO)
                       + edge_overshoot)
        if x <= left_limit:
            x = left_limit
            self._fling_vx = abs(self._fling_vx) * FLING_BOUNCE
        elif x >= right_limit:
            x = right_limit
            self._fling_vx = -abs(self._fling_vx) * FLING_BOUNCE
        if y <= 0:
            y = 0
            self._fling_vy = abs(self._fling_vy) * FLING_BOUNCE
        grounded = y >= ground_y
        if grounded:
            y = ground_y
        ...
        if (visual_key != getattr(self, '_airborne_last_visual_key', None)
            ...
            self._airborne_next_render_at = now + AIRBORNE_RENDER_MS / 1000.0
    ...
    def _climb_tick(self):
        ...
        self.root.geometry(f'{size}x{size}+{int(x)}+{int(y)}')
        self.update_image()
        self._climb_job = self.animations.schedule('climb', CLIMB_FRAME_MS, self._climb_tick)
    ...
    def _walk_tick(self):
        ...
        self._walk_job = self.animations.schedule('walk', WALK_STEP_MS, self._walk_tick)
    ...
    def _compute_landing_y(self, x=None, y=None):
        ...
        top = self.root.winfo_y() if y is None else int(y)
        feet = top + feet_h                   # 脚底当前屏幕 y
        x_left = self.root.winfo_x() if x is None else int(x)
        x_right = x_left + size
        desktop = self._desktop_bottom()
