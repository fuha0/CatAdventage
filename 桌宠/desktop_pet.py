...
FLING_MIN_SPEED = 700       # 低于该鼠标速度不触发（像素/秒）
FLING_SPEED_SCALE = 0.55    # 鼠标速度转换为基准帧移动距离
FLING_MAX_SPEED = 53        # 33ms 基准帧下的最大初速度
FLING_FRICTION = 0.955      # 33ms 基准帧的空气阻力
FLING_GROUND_FRICTION = 0.9  # 33ms 基准帧的地面摩擦力
FLING_GROUND_STOP_SPEED = 0.75  # 接地后低于该每帧速度即结束动画
FLING_GRAVITY = 1.13        # 33ms 基准帧的垂直重力
FLING_BOUNCE = 0.28         # 撞墙/落地反弹系数
FLING_SECOND_BOUNCE = 0.45  # 高落差落地后的第二次小弹跳
FLING_THIRD_BOUNCE = 0.55   # 高落差连续弹跳的第三次小弹跳
FLING_SECOND_BOUNCE_MIN_IMPACT_SPEED = 10.0  # 首次落地速度达到该值时启用连续弹跳
FLING_SECOND_BOUNCE_MIN_SPEED = 0.75  # 后续落地的最低反弹速度
